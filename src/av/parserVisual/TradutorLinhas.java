...
public class TradutorLinhas extends Thread {
	...
	/**
	 *  Método iniciado logo após thread ser instânciada na activity.
	 */
	public void run() {
		 ...
		 // terceira parte: lendo e executando os comandos
          ...
          while ((line = br.readLine()) != null) {
        	  ...
        	  if(stop==true){
				...
				//Caso encontre a palavra reservada inicio para o loop
	...
	public boolean procurarComandoE(String linha,String comando){
			...
			//Retirando todo o espaço em branco que existia na String linha.
		linha = linha.replace(" ", "");
		System.out.println("new :"+linha+"<fim");
		System.out.println(comando);
		
		if(linha.equals(comando)){
	...
	public boolean executarCodigo(String linha,BufferedReader br){
		...
		if(linha.contains("escreva(") || linha.contains("escreva (")){
			 escreva(linha);				
			}
		
		if(linha.contains("escreval(") || linha.contains("escreval (")){
		 escreval(linha);				
		}
		
		if((linha.contains("se") || linha.contains("entao")) && 
				(!linha.contains("escreva") && !linha.contains("<-") && !linha.contains("leia(") &&
				(!linha.contains("para") && !linha.contains("faca")) && !linha.contains("//") &&
				!linha.contains("fimse") &&  !linha.contains("senao"))){
	...
	private void escreval(String linha){		
		...
		if (linha.contains("escreval(")){
			...
			inicio = linha.indexOf("escreval(")+9;
		}else{
			
			inicio = linha.indexOf("escreval ("+10);
		}
    ...
    private void escreva(String linha){		
    	...
    	if(linha.contains("escreva(")){
    		inicio = linha.indexOf("escreva(")+8;
    	}else{
    		inicio = linha.indexOf("escreva (")+9;
    	}
